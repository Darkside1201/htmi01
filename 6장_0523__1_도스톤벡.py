...
# 리스트 내용 삽입, 삭제
list[2] = (1,2,3)  # 2번지에 튜플 삽입
print("list1 내용 : ", list1)

del list1[0]   # 0번지 삭제
print("text1 첫번째 요소 삭제 후 내용 : ", list1)

# 합계 계산 sum() 함수 사용.
print("list1의 요소들의 합 : ", sum(list1)) # 오류 = 데이터
...
# 좋아하는 과일 리스트를 생선하시오. 좋아하는 순서대로.
fruit = ['수박', '딸기', '복숭아', '망고']
print("내가 좋아하는  과일 리스트 : ", fruit)

# 3번째로 좋아하는 과일을 '두리안'으로 바꾸시오.
fruit[3] = '두리안'
print("내가 좋아하는 과일 리스트(두리안 교체) : ", fruit)

#마지막에 '바나나'를 추가하시오
fruit("내가 좋아하는 과일 리스트(바나나 추가) : ", fruit)

#딸기를 삭제하시오. 수박, 망고
fruit.pop(1)  #index(주소)로 접근
fruit("내가 좋아하는 과일 리스트(딸기 삭제) : ", fruit)

#딸기를 삭제하시오. 
fruit.remove(바나나)  #index(주소)로 접근
fruit("내가 좋아하는 과일 리스트(딸기 삭제) : ", fruit)
...
# 역순으로 정리하시오.(내림차순)
animal. reverse()
print("역순으로 줄을 서시오~~ ", animalk)

# 몇 마리의 동물들의 줄을 서 있는지 알려주시오.
